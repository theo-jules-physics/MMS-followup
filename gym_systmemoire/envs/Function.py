...
def plot_paysage(func, val_lim, x_name, y_name, n=100, marq='.-'):
    """
    Plot the landscape of func between the limit values val_lim with n points.
    ...
    func : callable
        Function to plot.
    val_lim : tuple
        Limit values (min, max) for the x axis.
    x_name : str
...
class EQD_mouvement():
    ...
    def plot_evol_pos(self):
        """
        Plot the evolution of position for each mass over time.
        ...
        for k, x in enumerate(self.X_sol[:self.n, :]):
            plt.plot(self.t_sol, x, '.-', label=f'Masse n° {k}')
        plt.legend()
        return figure, ax

    def plot_evol_diff_pos(self):
        """
        Plot the evolution of position differences for each mass over time.
