...
class A2C(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def update(self):
        with torch.no_grad():
            _, next_value = self.model(self.states[-1])   #self.states[-2:]
            #print('state[-1] : ', self.states[-1].shape)
            next_value = next_value[:, 0]

        self._compute_returns(next_value)
        pout, values = self.model(self.states[:-1].reshape(-1, *self.obs_shape)) #self.states[-3:-1]
        #print(self.states[-3])
        #print(self.states[-2])
        #print(self.states[-3:-1])
    ...
    def _batch_act_train(self, batch_obs):
        ...
        self.states[self.t - self.t_start] = statevar #self.states[-2:]
